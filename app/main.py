...
async def process_pdf_task(job_id: int, file_path: Path, db: Session):
    ...
    try:
        ...
        metadata, tier = await ai_service.process(text=raw_text, image_bytes=image_bytes)
        ...
        # 6. Physical Split (even if 1 document, we use split to save to processed folder)
        output_paths = pdf_service.split_pdf(
            source_path=file_path,
            page_ranges=[[0]], # Simple 1-page logic for now, can be expanded
